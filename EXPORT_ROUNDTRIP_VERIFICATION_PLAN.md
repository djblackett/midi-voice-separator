...
| Percussion                                | A marked exported percussion track can reimport as generic `voice-N`.                                      | Add explicit app-exported voice-role metadata and restore the `PERCUSSION` role deterministically.                                  |
...
| Unlisted note voices                      | Exporter writes an `Unassigned` track but reports `voices + 1` tracks.                                     | Report actual emitted track count; validation/report explicitly identifies the extra exported voice track.                          |
| Zero-length notes                         | Current same-tick sort emits note-off before its note-on, producing parser warnings/repair.                | Repair ordering: old note-offs, zero-length note-on/off pairs, then ordinary note-ons.                                              |
