...
| Empty voices                              | Marked tracks are rebuilt even without notes, retaining their label and role.                              | Keep marked-track recovery separate from generic heuristic import.                                                                  |
| Percussion                                | A versioned marker preserves the explicit `PERCUSSION` role even for an empty marked track.                | Treat role metadata as authoritative for marked exports; generic imports still infer channel 10.                                    |
